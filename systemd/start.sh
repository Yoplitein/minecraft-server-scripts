...
neoforgeJVMFlags=( libraries/net/neoforged/neoforge/*/unix_args.txt )
...
elif [ -e $neoforgeJVMFlags ]; then
    echo "Launching using NeoForge method ($neoforgeJVMFlags)"
    exec env java -Xms${MEM_MIN} -Xmx${MEM_MAX} $JVM_ARGS @$neoforgeJVMFlags $MC_ARGS
